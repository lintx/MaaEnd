...
def build_transfer_cases(catalog: dict, zh_cn: dict) -> list[dict]:
    cases = []
        ...
        cases.append(
            {
                ...
                "pipeline_override": {
                    ...
                    "ItemTransferFindItemInRepo": _item_id_override(item["id"]),
                    "ItemTransferFindItemInBag": _item_id_override(item["id"]),
                    "ItemTransferFindItemInBagReturn": _item_id_override(item["id"]),
                },
            }
        )
    return cases
...
def update_item_transfer_task(task: dict, cases: list[dict]) -> dict:
    updated = deepcopy(task)
    updated["option"]["WhatToTransfer"]["cases"] = cases
    return updated
...
def generate_item_transfer_task(catalog_path: Path, locale_path: Path, task_path: Path) -> int:
    ...
    cases = build_transfer_cases(catalog, zh_cn)
    updated = update_item_transfer_task(task, cases)
